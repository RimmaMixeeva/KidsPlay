...
@Composable
fun ColorGameSecondLevelsScreen(viewModel: ColorGameViewModel) {
        ...
        if (colorLevels.value.size > 29) {
            LazyVerticalGrid(
                ...
                items(15) { i ->
                    ColorLevel(
                        {
                            if (colorLevels.value[i + 15].isLevelOpened) {
                                viewModel.startLevel(colorLevels.value[i + 15].levelNumber)
                            }
                        },
                        colorLevels.value[i + 15]
                    )
                }
            }
        }
