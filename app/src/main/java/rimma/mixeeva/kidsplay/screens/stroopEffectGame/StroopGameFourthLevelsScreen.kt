...
@Composable
fun StroopGameFourthLevelsScreen(viewModel: StroopEffectGameViewModel) {
    ...
    Box(contentAlignment = Alignment.Center) {
        Image(
            painter = painterResource(id = R.drawable.hell),
            contentDescription = "hell",
            modifier = Modifier.fillMaxSize(),
            contentScale = ContentScale.FillBounds
        )

        if (stroopLevels.value.size > 59) {
            LazyVerticalGrid(
                ...
                items(15) { i ->
                    StroopEffectLevel(
                        {
                            if (stroopLevels.value[i + 45].isLevelOpened) {
                                viewModel.startLevel(stroopLevels.value[i+45].levelNumber)
                            }
                        },
                        stroopLevels.value[i + 45]
                    )
                }
            }
        }
